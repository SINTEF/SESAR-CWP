...
export default observer(function ChangeBearingPopup(properties: {
	...
	const submit = (): void => {
		const newBearingNumber = Math.max(
			-360,
			Math.min(Number.parseInt(newBearing, 10) || 0, 360),
		);
		...
		posthog?.capture("bearing_changed", {
			...
			new_bearing: newBearingNumber,
			bearing_change: newBearingNumber - lastKnownBearing,
		...
		handlePublishPromise(persistACCBearing(aircraftId, newBearingNumber));
		close();
	};
	...
	return (
		<div className="bg-gray-700/50 rounded-sm p-1 w-[124px] relative">
			...
			>
				...
				<input
					className="input input-bordered input-xs bg-transparent text-white w-[4.4em]"
					type="text"
					inputMode="numeric"
					pattern="-?[0-9]*"
					value={newBearing}
