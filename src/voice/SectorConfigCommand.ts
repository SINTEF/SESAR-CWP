...
function convertISectorModelToTurfPolygon(
	sector: ISectorModel,
): Feature<Polygon> {
	const coordinates = sector.sectorArea.map(
...
function findSectorWithLargestIntersection(
	...
	const areas = otherSectors.map((s) => {
		...
		const intersection = intersect(
			featureCollection([sectorPolygon, aboveSectorPolygon]),
		);
	...
	const largestSector = areas.reduce((previous, current) =>
		previous.area > current.area ? previous : current,
	);
...
function findAbove(
	...
	const above = otherSectors.filter(
		(s) => s.bottomFlightLevel === sector.topFlightLevel,
	);
...
function findBelow(
	...
	const below = otherSectors.filter(
		(s) => s.topFlightLevel === sector.bottomFlightLevel,
	);
...
function findCloseSectorWithVerticalOverlap(
	sector: ISectorModel,
	otherSectors: ISectorModel[],
	direction: "above" | "below" | "west" | "east",
): ISectorModel | undefined {
	...
	const otherSectorsWithHeight = otherSectors
		...
		.map((s) => {
			...
			const distance = Math.sqrt(
				(sectorCenterOfMass[0] - center[0]) ** 2 +
					(sectorCenterOfMass[1] - center[1]) ** 2,
			);
		...
		.filter(
			(s) =>
				s.center[indexToCompare] > sectorCenterOfMass[indexToCompare] ===
				reverse,
		);
...
function findSectorWithCWPNumber(
	...
	const matchingSector = sectorsWithCWP.find(
		(s) => s.cwp === `CWP${cwpNumber}`,
	);
